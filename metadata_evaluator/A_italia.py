...
def A2():
    ...
    if is_independent():
        scored_point += 1
        mprint("LOG: is independent always pass for dati italia")
    else:
...
def guidelines_a(payload):
    ...
    if "resources" not in payload["result"]:
    ...
    max_point = len(payload["result"]["resources"]) * 3
    mprint("LOG: found " + str(len(payload["result"]["resources"])) + " distribution")
    scored_point = 0
    max_point_found = 0
    ...
    for idx, distribution in enumerate(payload["result"]["resources"]):
        ...
        if "download_url" in distribution \
            ...
            scored_point += 1
            if type(distribution["download_url"]) is str:
                distribution_url = [distribution["download_url"]]
            else:
                distribution_url = distribution["download_url"]

            for url in distribution_url:
                try:
            ...
            if type(distribution["access_url"]) is str:
            ...
            if "url" in distribution \
                ...
                print("LOG: found access url")
                scored_point += 0.3
                if type(distribution["url"]) is str:
                ...
                for url in distribution_url:
                    try:
                        ...
                        if response.status_code in range(200, 400) or response.status_code == 401:
                            print("LOG: url working")
                            max_point_found = max(max_point_found, 0.3)
                            if response.status_code != 401:
                                print("LOG: url accessible")
                                max_point_found = max(1, max_point_found)
                                break
                ...
                if max_point_found == 0.3:
                    reporter.add_to_report("--per la distribuzione: " + distribution["id"] +
                                           " è fornito un url valido ma non accessibile. "
                                           "L'URL sembra un url mentre dovrebbe essere download_url. "
                                           "Il punteggio verrà ridotto a un terzo")
                if max_point_found == 0:
                    reporter.add_to_report("--per la distribuzione: " + distribution["id"] +
                                           " è fornito un url non valido.")

...
def R1():
    ...
    max_point = 1
    scored_point = 1
    mprint("LOG: always pass for dati.gov.it")
...
def R1_1(payload):
    ...
    for idx, distribution in enumerate(payload["result"]["resources"]):
        ...
        elif "license_type" in distribution and distribution["license_type"] is not None:
            mprint("LOG: for distribution " + str(idx + 1) + " found license")
            scored_point += 1

            if distribution["license_type"] == vocabularies["license"]["vocabulary_format"][
                                               :len(distribution["license_type"])] \
                    and requests.get(distribution["license_type"]).status_code in range(200, 400):
                mprint("LOG: license respect vocabulary")
                scored_point += 1
...
def R1_2(payload):
    ...
    if ("publisher" in payload["result"] and payload["result"]["publisher"] is not None) \
...
def R1_3(eu_dataset_id):
    ...
    community_score = community_standard(eu_dataset_id)
...
def guidelines_r(payload):  # TO IMPLEMENT SHACL VLIDATIO SEE GUIDELINES
    ...
    max_point += 1
    mprint("LOG: compliance to community standard always true for dati.gov.it")
    scored_point += 1
    ...
    for distribution in payload["result"]["resources"]:
        ...
        if "byte_size" in distribution and distribution["byte_size"] is not None:
            ...
            scored_point += 1
        elif "size" in distribution and distribution["size"] is not None:
            mprint("LOG: size found")
            scored_point += 0.5
            reporter.add_to_report("--byte_size not implemented but found size. half point")
        else:

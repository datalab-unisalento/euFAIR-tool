...
def F1(payload):
    ...
    if "resource" in payload["result"] \
        ...
        log.log(f"LOG: F1 -> max point = {max_point} | point scored = {point_scored}")
    ...
    try:
        ...
        for _property in schema_properties:
            ...
            if _property in metadata_properties and metadata_properties[_property] != {} \
                ...
                reporter.add_to_report("--la proprietà " + _property + " non sembra essere stata dichiaratà")
...
def F4(payload):
    ...
    try:
        ...
        log.log(f"LOG: F4 -> max point = {max_point} | point scored = {point_scored}")
